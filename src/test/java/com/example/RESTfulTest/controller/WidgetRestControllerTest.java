...
class WidgetRestControllerTest {
    ...
    void getWidgetPorId() throws Exception {
        // Setup our mocked service
        Widget widget = new Widget(1l, "Widget calendario", "calendario con sus fechas y horas", 1);
        ...
        mockMvc.perform(get("/rest/widget/{id}", 1L))
                ...
                // Validate the returned fields
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.name", is("Widget calendario")))
                .andExpect(jsonPath("$.description", is("calendario con sus fechas y horas")))
                .andExpect(jsonPath("$.version", is(1)));
    }

    @Test
    @DisplayName("PUT /rest/widget/1")
    void modificarWidget() throws Exception {
        // Setup our mocked service
    ...
    void modificarWidgetNotFound() throws Exception {
        // Setup our mocked service
        Widget widgetToPut = new Widget("New Widget", "This is my widget");
        doReturn(Optional.empty()).when(service).findById(1L);
